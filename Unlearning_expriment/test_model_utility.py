...
from transformers import set_seed, AutoConfig, RobertaTokenizer, LlamaTokenizer
...
def load_model_tokenizer(args, model_name, model_path):
    ...
    if args.lora_path:
        model = PeftModel.from_pretrained(model, args.lora_path, torch_dtype=torch.bfloat16)
    ...
    # Token ID config consistent with eval_o3.py
...
def generate_code_for_tasks(args, except_tasks, save_file):
    ...
    if getattr(args, 'ood_weights', None):
        # --- OOD mode: LlamaForCausalLM_ood + hacked PeftModel with init_oodweight() ---
        print("\n=== Loading LlamaForCausalLM_ood + LoRA + OOD ===")
        ...
        model = LlamaForCausalLM_ood.from_pretrained(
            base_path, config=config, torch_dtype=torch.bfloat16, device_map="auto", low_cpu_mem_usage=True)
        model = PeftModel.from_pretrained(model, args.lora_path, torch_dtype=torch.bfloat16)
        model.init_olora(orthogonal_loss=False, olora_weights={})
        model.init_active_adapters_d(active_adapters_d=['default'])
        ...
        ood_model_ref = model
        generate_code_fn = lambda a, p: sample_code_from_llm(a, p, model, tokenizer)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        if ood_model_ref is not None and ood_components is not None:
            ...
            w_tensor = torch.tensor([w], dtype=torch.bfloat16).to(device)
            ood_model_ref.init_oodweight(ood_weight=[1, w_tensor])
