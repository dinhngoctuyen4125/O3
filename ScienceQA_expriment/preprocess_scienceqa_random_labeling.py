...
# topics = ['biology', 'physics', 'chemistry'] # 'biology', 'physics', 'chemistry', 'economics', 'earth-science'
topics = ['biology', 'physics', 'chemistry', 'economics', 'earth-science']
...
save_path = "./data/scienceqa_random_force_5/"
...
for t in topics:
    ...
    for split in splits:
        ...
        for n in tqdm(range(len(datasets_scienceqa[split]))):
            if datasets_scienceqa[split][n]['image'] == None and datasets_scienceqa[split][n]['topic'] == t:
                ...
                data_sample['real_answer'] = answer
                data_sample['real_input'] = ""
                data_sample['real_instruction'], data_sample['real_output'], _ = create_one_example(prompt_format,
                                                                                          question,
                                                                                          context,
                                                                                          choice,
                                                                                          answer,
                                                                                          ...
                                                                                          solution,
                                                                                          test_example=False)
                input = data_sample['real_input']
                instruction = data_sample['real_instruction']
                output = data_sample['real_output']
                ...
                data_sample['real_text_full'] = full_prompt
                data_sample['real_text_input'] = input_prompt

                original_answer = answer
                option_len = len(datasets_scienceqa[split][n]["choices"])
                randomlist = [options[x] for x in range(len(options))]
                randomlabel = random.choice(randomlist)
                answer = randomlabel
        ...
        with open(save_path + 'scienceqa_' + t + '_' + split + '_random_force.json', 'w') as json_file:
    ...
    datasets_wo_text_scienceqa.save_to_disk(save_path + 'scienceqa_' + t + '_all_random_force.hf')
    datasets_wo_text_scienceqa = load_from_disk(save_path + 'scienceqa_' + t + '_all_random_force.hf')
